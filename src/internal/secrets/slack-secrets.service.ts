...
type AppId = string;
type AppSecrets = { signingSecret: string };

export class SlackSecretsService {
  private static readonly SLACK_SECRET_ARN = process.env.SLACK_SECRET_ARN!;
  private static readonly SIGNING_SECRET_REGEX =
    /^app\/(?<appId>[a-zA-Z0-9]+)\/signing-secret$/;
  ...
  private async retrieveAndParseSecret() {
    ...
    return Object.entries(secretObject).reduce<Record<AppId, AppSecrets>>(
      (curr, [key, value]) => {
        const match = SlackSecretsService.SIGNING_SECRET_REGEX.exec(key);
